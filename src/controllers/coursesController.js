...
export default {
  create_histories: (req, res, next) => {
    recordStartTime.call(req);
    try {
      const { coursesId, usersId, status } = req.query;
      const param = { coursesId, usersId, status };

      // console.log("provinceService param: ", param)
      coursesService
        .create_histories(param)
        .then(data => {
          res.send(data);
  ...
  get_one: (req, res, next) => {
    ...
    try {
      const { id } = req.params;
      const { attributes, usersId } = req.query;
      const param = { id, attributes, usersId };
  ...
  updateOrder: (req, res, next) => {
    recordStartTime.call(req);
    try {
      console.log('updateOrder Controllers');
      const { id } = req.params;
